...
func TestValidateHistoryRejectsFutureVersions(t *testing.T) {
	...
	if err := validateHistory(map[int]string{1: files[0].digest, 2: files[1].digest, 3: files[2].digest, 4: "future"}, files); !errors.Is(err, ErrInvalidHistory) {
...
func TestMigrationHelpersAndSQLMockApplyVerify(t *testing.T) {
	...
	if got := nextVersion(map[int]string{1: files[0].digest, 2: files[1].digest, 3: files[2].digest}); got != 4 {
	...
	verifyMock.ExpectQuery(`SELECT version, sha256 FROM public.schema_migrations`).WillReturnRows(sqlmock.NewRows([]string{"version", "sha256"}).AddRow(files[0].version, files[0].digest).AddRow(files[1].version, files[1].digest).AddRow(files[2].version, files[2].digest))
