...
全局、schema 级、列级、routine/`EXECUTE`、启用角色的越权权限和 grant option 都会使 Bootstrap
...
schema、列级或 routine/`EXECUTE` 权限，也不拥有 schema DDL 权限。运行时
...
| Bootstrap | `TRPC_CONTROL_PLANE_DRIVER=mysql` 选择 MySQL；双 DSN 账号/数据库分离校验、14 张表逐表完整 DML 白名单（缺失或额外权限、routine/`EXECUTE`、启用角色和 grant option 均拒绝）、未知驱动、缺 DSN、迁移失败和重启 rediscovery 由 Bootstrap/sqlmock 契约 fail-closed，live job 验证受限应用账号可运行 Repository |
