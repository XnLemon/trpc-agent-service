-- Package-owned base table schema for trpcservice/runtime/queue/postgres.
-- Keep cross-package foreign keys, functions, triggers, grants, and later
-- evolution steps in the migration orchestrator.

CREATE TABLE IF NOT EXISTS public.runtime_execution_queue (
...
CREATE INDEX IF NOT EXISTS runtime_execution_queue_ready_idx
    ON public.runtime_execution_queue (tenant_id, next_attempt_at, created_at)
    WHERE status IN ('queued','retryable');

CREATE INDEX IF NOT EXISTS runtime_execution_queue_lease_idx
