...
func TestAgentRepositoryListFiltersAndPaginates(t *testing.T) {
	...
	items, cursor, err := NewRepository(db).List(context.Background(), first.TenantID, " PAYMENTS ", string(agent.StatusDraft), "1", 1)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAgentRepositoryListFailureCases(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		call  func(*AgentRepository, *agent.App) error
		want  error
	}{
		{
			name: "invalid cursor",
			call: func(repository *AgentRepository, app *agent.App) error {
		...
		{
			name: "missing rows",
			setup: func(mock sqlmock.Sqlmock, app *agent.App) {
		...
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock, app *agent.App) {
				invalid := app.Clone()
				invalid.Status = agent.Status("unknown")
...
func TestAgentRepositoryListRevisionsFiltersAndPaginates(t *testing.T) {
	...
	revisions := []*agent.Revision{first, second, nonMatching, published}

	db, mock := newAgentListMock(t)
	expectAgentRevisionList(t, mock, app, revisions)

	items, cursor, err := NewRepository(db).ListRevisions(context.Background(), app.TenantID, app.AppID, " PAYMENTS ", string(agent.RevisionStateDraft), "1", 1)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAgentRepositoryListRevisionsReturnsEmptyPagePastEnd(t *testing.T) {
	...
	db, mock := newAgentListMock(t)
	expectAgentRevisionList(t, mock, app, []*agent.Revision{revision})
...
func TestAgentRepositoryListRevisionsFailureCases(t *testing.T) {
	...
	for _, tc := range []struct {
		name  string
		setup func(*testing.T, sqlmock.Sqlmock, *agent.App)
		call  func(*AgentRepository, *agent.App) error
		want  error
	}{
		{
		...
		},
		{
			...
			setup: func(_ *testing.T, mock sqlmock.Sqlmock, app *agent.App) {
...
func agentAppRows(values ...*agent.App) *sqlmock.Rows {
...
func repeatedAgentApps(value *agent.App, count int) []*agent.App {
	values := make([]*agent.App, count)
	for i := range values {
		clone := value.Clone()
		values[i] = &clone
	}
...
func agentListRevisions(t *testing.T, app *agent.App, count int) []*agent.Revision {
	t.Helper()
	values := make([]*agent.Revision, count)
	for i := range values {
...
func expectAgentRevisionList(t *testing.T, mock sqlmock.Sqlmock, app *agent.App, values []*agent.Revision) {
	t.Helper()
...
func newListRevision(t *testing.T, app *agent.App, number int64, published bool, description, globalInstruction string) *agent.Revision {
	t.Helper()
