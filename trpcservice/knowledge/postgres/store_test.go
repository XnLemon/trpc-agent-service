...
func TestStoreUsesTenantPredicateAndRoundTripsDocument(t *testing.T) {
	...
	store, err := knowledgepostgres.New(db, "t_00000000000000000000000000", knowledgepostgres.WithAppID("app_00000000000000000000000000"), knowledgepostgres.WithDimension(2))
	if err != nil {
		t.Fatal(err)
	}
	created := time.Unix(10, 0).UTC()
	updated := created.Add(time.Minute)
	doc := &document.Document{ID: "doc-a", Name: "Guide", Content: "hello", Metadata: map[string]any{"kind": "guide", "_trpc_app_id": "app_00000000000000000000000000"}}
	mock.ExpectQuery("INSERT INTO public.runtime_knowledge_document").
		WithArgs("t_00000000000000000000000000", "app_00000000000000000000000000", "doc-a", "Guide", "hello", "", sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg()).
	...
	mock.ExpectQuery("SELECT document_id, name, content, embedding_text, metadata, embedding, created_at, updated_at").
		WithArgs("t_00000000000000000000000000", "app_00000000000000000000000000", "doc-a").
		WillReturnRows(sqlmock.NewRows([]string{"document_id", "name", "content", "embedding_text", "metadata", "embedding", "created_at", "updated_at"}).
			AddRow("doc-a", "Guide", "hello", "", []byte(`{"kind":"guide","_trpc_app_id":"app_00000000000000000000000000"}`), []byte(`[1,0]`), created, updated))
...
func TestStoreSearchFiltersAndScoresPersistedRows(t *testing.T) {
	...
	defer func() { _ = db.Close() }()
	store, err := knowledgepostgres.New(db, "t_00000000000000000000000000", knowledgepostgres.WithAppID("app_00000000000000000000000000"), knowledgepostgres.WithDimension(2), knowledgepostgres.WithMaxResults(5))
	if err != nil {
		t.Fatal(err)
	}
	created := time.Unix(10, 0).UTC()
	mock.ExpectQuery("SELECT document_id, name, content, embedding_text, metadata, embedding, created_at, updated_at").
		WithArgs("t_00000000000000000000000000", "app_00000000000000000000000000").
		WillReturnRows(sqlmock.NewRows([]string{"document_id", "name", "content", "embedding_text", "metadata", "embedding", "created_at", "updated_at"}).
			AddRow("doc-a", "A", "alpha", "", []byte(`{"kind":"guide","_trpc_app_id":"app_00000000000000000000000000"}`), []byte(`[1,0]`), created, created).
			AddRow("doc-b", "B", "beta", "", []byte(`{"kind":"note","_trpc_app_id":"app_00000000000000000000000000"}`), []byte(`[0,1]`), created.Add(time.Second), created.Add(time.Second)))
...
func TestStoreRedactsDatabaseErrorsAndClosesWithoutClosingPool(t *testing.T) {
	...
	defer func() { _ = db.Close() }()
	store, err := knowledgepostgres.New(db, "t_00000000000000000000000000", knowledgepostgres.WithAppID("app_00000000000000000000000000"))
	if err != nil {
		t.Fatal(err)
	}
	mock.ExpectQuery("SELECT document_id, name, content, embedding_text, metadata, embedding, created_at, updated_at").
		WithArgs("t_00000000000000000000000000", "app_00000000000000000000000000", "missing").
