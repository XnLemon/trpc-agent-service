...
func TestPostgreSQLPureBoundaryHelpers(t *testing.T) {
	...
	if err := validateAgentMetadata(agent.ChangeMetadata{ActorType: "a", ActorID: "b", Reason: strings.Repeat("x", 1001), CorrelationID: "d"}); !errors.Is(err, agent.ErrInvalid) {
		t.Fatalf("long agent metadata error = %v", err)
	}
	if got := agentStatusEventType(agent.StatusSuspended); got != agent.ChangeSuspended {
		t.Fatalf("suspended event = %s", got)
	}
	if got := agentStatusEventType(agent.StatusActive); got != agent.ChangeResumed {
		t.Fatalf("active event = %s", got)
	}
	if got := agentStatusEventType(agent.StatusDisabled); got != agent.ChangeDisabled {
		t.Fatalf("disabled event = %s", got)
	}
	value := int64(4)
	if clone := cloneAgentInt64(&value); clone == nil || clone == &value || *clone != value || cloneAgentInt64(nil) != nil {
		t.Fatal("agent integer clone is not defensive")
	}
	if got := *agentInt64(value); got != value || nullableInt64(sql.NullInt64{Int64: value, Valid: true}) == nil || nullableInt64(sql.NullInt64{}) != nil {
		t.Fatal("agent integer helpers are incorrect")
	}
	now := time.Now().UTC()
	if pointer := timePointer(now); pointer == nil || !pointer.Equal(now) || !maxTime(now, now.Add(time.Second)).Equal(now.Add(time.Second)) || !maxTime(now.Add(time.Second), now).Equal(now.Add(time.Second)) {
		t.Fatal("agent time helpers are incorrect")
	}
